...
export default function PastGuestsPills() {
  ...
  useGSAP(
    () => {
      ...
      tl.to(".pg-reveal", { y: 0, opacity: 1, filter: "blur(0px)", duration: 1 }, 0)
        .set(".pg-reveal", { filter: "none" })
        .to(
          ".pg-pill",
  ...
  return (
    <section ref={rootRef} className="px-6 pb-2 pt-18 sm:px-11">
      <h2 className="pg-reveal reveal-fade mb-6 font-archivo text-[clamp(1.5rem,1.1rem+1.8vw,2.125rem)] font-black leading-none uppercase">
        Past <span className="inline-block -rotate-1 bg-mango px-2">guests</span>
      </h2>
      <div className="flex flex-wrap items-center gap-3 sm:gap-3.5">
        {GUESTS.map((g) => (
          <span
            key={g}
            className="pg-pill inline-flex shrink-0 items-center justify-center whitespace-nowrap rounded-full border-2 border-ink bg-white px-4.5 py-2.5 font-sans text-[13.5px] font-extrabold opacity-0 shadow-[3px_3px_0_#1A1714] will-change-transform"
